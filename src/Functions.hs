...
{-flip elem pega o elemento ps (mina) e verifica se ele está presente na lista neighbors (a,b), que são os elementos vizinhos da casa analisada. Caso sejam encontradas minas, elas são filtradas da lista pela função filter e contadas pela função length, obtendo-se o número de minas nas casas vizinhas-}
